...
function NewFactForm({ setFacts, setShowForm }) {
  ...
  return (
    <form className="fact-form" onSubmit={handleSubmit}>
      <input disabled={isUploading} type="text" placeholder="Share a fact with the world..." value={text} onChange={(e) => setText(e.target.value)} />
      <span>{200 - textLength}</span>
      <input disabled={isUploading} type="text" placeholder="Trustworthy source..." value={source} onChange={(e) => setSource(e.target.value)} />
      <select disabled={isUploading} value={category} onChange={(e) => setCategory(e.target.value)}>
    ...
    <aside>
      <ul>
        <li className="category">
          <button onClick={() => setCurrentCategory('all')} className="btn btn-all-categories">All</button>
        </li>
        {CATEGORIES.map((cat) => (
